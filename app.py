...
Original file is located at
    https://colab.research.google.com/drive/1ubk0xh_-7o9aKdb2VaSKO0gD9yGJKv7i
"""
...
if not product_reviews.empty:
    # Truncate and clean the reviews
    product_reviews["Review"] = product_reviews["Review"].apply(
    ...
    # Visualize sentiment results with a bar chart
    fig = px.bar(sentiment_df, x='label', title='Sentiment Analysis Results')
    st.plotly_chart(fig)
else:
    st.write("No reviews data available for this product.")

# Check columns to ensure 'discount' exists
st.write("Columns in product_data:", product_data.columns)

# Ensure column names are clean (no leading/trailing spaces)
product_data.rename(columns=lambda x: x.strip(), inplace=True)

# Check if 'discount' exists before processing
if 'discount' not in product_data.columns:
    st.write("The 'discount' column is missing from the product data.")
else:
    ...
    # Convert 'discount' to numeric and clean data
